...
process.on('message', (m) => {
	...
	if (m.transaction == false) {
		...
		mail.transactional(m, function (err, result) {
			if (err) {
				throw err;
			} else {
				var object = {};
				object["id"] = result['Messages'][0]['To'][0]['MessageID'];
				object["from"] = m.From.email;
				object['html'] = fs.readFileSync(m.htmlPath, 'utf8');
				object['text'] = fs.readFileSync(m.textPath, 'utf8');
				var email = m.To.email;
				mainDb.updateTransactionalDetails(email,object,function (err, results) {
					if (err) {
						throw err;
					} else {
						console.log(results);
